...
class NetUtils():
  ...
  # Given an IP with a mask (in CIDR format), return network in CIDR format
  @staticmethod
  def net_of_ip(src_ip_cidr):
    (ip, mask) = NetUtils.net_mask(src_ip_cidr)
    src_ip_int = NetUtils.ipv4_to_int(ip)
    net_int = pow(2, mask+1) -1  << (32-mask)
    net_int &= int(src_ip_int)
    return NetUtils.ipv4_to_str(net_int)+"/"+str(mask)
