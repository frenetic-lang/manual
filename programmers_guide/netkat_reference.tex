...
\python{ryu.packet.ether_types}.  REST-based NetKAT requires them to be specified in
decimal.
...
\python{IP4Dst} matches packets with a particular IP destination address, or destination network.  It only matches
IP v4 packets, as per Openflow 1.0.  

\python{IP4DstEq} cannot accept a list of values as other predicates can.  
...
\python{IP4DstEq("192.168.57.0", 24)} matches IP addresses in subnet \python{192.168.57.0/24} -- e.g. 
...
\python{IP4Src} matches packets with a particular IP source address, or source network.  It only matches
IP v4 packets, as per Openflow 1.0.  

\python{IP4SrcEq} cannot accept a list of values as other predicates can.  
Instead, you can specify a subnet -- a range
of IP addresses using a bit mask value.  This is identical to using CIDR format, so that 
\python{IP4SrcEq("192.168.57.0", 24)} matches IP addresses in subnet \python{192.168.57.0/24} -- e.g. 
IP addresses \python{192.168.57.1} to \python{192.168.57.255}.  Leaving out the subnet mask in Python 
or Raw NetKAT is equivalent to specifying a mask of 32 - meaning all 32 bits of the IP address are used
in the match.  (The 32 must be explicitly specified in REST-based NetKAT).
...
\python{id} is a \emph{policy} that accepts all packets.  It is equivalent to \python{Filter(true)}.
...
\netkat{SetPort}, \netkat{SendToController}, or \netkat{SendToQuery} in a sequence, the last one will
win.  If you want a packet to go to more than one destination, use \netkat{Union} between them.    
...
\python{SetIP4Dst} sets the IP v4 destination address.  Note that unlike the \netkat{IP4DstEq} predicate,
...
\python{SetIP4Src} sets the IP v4 destination address.  Note that unlike the \netkat{IP4SrcEq} predicate,
there is no option for masking.  You must set a fixed IP address with all bits present.    
...
\python{SetPort} is the only modification policy that can take a list of arguments.  When more than one
port is specified, there is an implicit \netkat{Union} between them -- in other words, packets are copied
