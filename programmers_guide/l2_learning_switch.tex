...
\begin{itemize}
\item The \netkat{__init__} constructor must call the superclass constructor 
\python{frenetic.App.__init__} to properly initialize.
...
We'll add this code to LearningApp1 (the new program listed in \codefilename{l2_learning_switch/learning2.py}):
...
If that's true, why didn't Frenetic erase the \netkat{SendToController} policy 
...
\item Since the \netkat{SendToController} policy is still present, the second packet came to the controller, causing a second rule regeneration.
