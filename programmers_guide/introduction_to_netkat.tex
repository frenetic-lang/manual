...
One special case is \netkat{SwitchEq}, matching a switch id, which we'll talk about in a second.  
...
policies as a superset of OpenFlow actions.  NetKAT predicates like \netkat{SwitchEq} and policies like \netkat{Union} and
\netkat{Seq} have no equivalents in OpenFlow, but Frenetic compiles these down to OpenFlow tables.  

You hand Frenetic a NetKAT program through the \netkat{update} command.  Other commands like \netkat{pkt_out} send
packets directly to switches.  In the other direction, Frenetic provides hooks to handle events coming from the
switch.  A \netkat{packet_in} event fires off a handler you write for the \netkat{packet_in} hook.  
